...
enum EndPoint {
    ...
    /// for fetching article using source
    case source
    case country
    case topHeadline
    ...
    func makeURL(with parameters: [String : String]) -> URL {
        ...
        let strParams = paramsToString(parameters: parameters)
        let urlStr = Self.baseURL + "\(makePath())?\(strParams)"
        return URL(string: urlStr)!
    }
    
    func makeURLRequest(with parameters: [String : String],
    ...
    private func paramsToString(parameters: [String: String]) -> String {
        let parameterArray = getParams(parameters: parameters).filter { !$0.value.isEmpty }.map { key, value in
    ...
    private func getParams(parameters: [String: String]) -> [String: String] {
        switch self {
        ...
        case .articles, .language:
            ...
            return [ // find more info at https://newsapi.org/docs/endpoints/everything
                ...
                "from": parameters["from"] ?? "\(Service.getIso8601DateByWeek(weekCount: -1))", // A date and optional time for the oldest article allowed. This should be in ISO 8601 format (e.g. 2020-04-25 or 2020-04-25T02:36:43) Default: the oldest according to your plan.
                "to": parameters["to"] ?? "",
                ...
                "page": parameters["page"] ?? "1", // (Int) Use this to page through the results.
            ]
        case .country, .topHeadline, .category: // /top-headlines has these parameters
            return [
                ...
                //                "from": parameters["from"] ?? "\(Service.getIso8601DateByWeek(weekCount: -2))", //A date and optional time for the oldest article allowed. This should be in ISO 8601 format (e.g. 2020-04-25 or 2020-04-25T02:36:43) Default: the oldest according to your plan.
                "q": parameters["q"] ?? "", // Keywords or a phrase to search for.
                "pageSize": parameters["pageSize"] ?? "20",
                // The number of results to return per page (request). 20 is the default, 100 is the maximum.
                "page": parameters["page"] ?? "1", // Use this to page through the results if the total results found is greater than the page size.
            ]
